...
where abs(deviation_pct) > ${i.f_threshold.value??25}
  and cast(fiscal_year as varchar) like '${i.f_fy.value??"%"}'
order by abs(deviation_pct) desc)`;l.anomaly_summary_data&&(l.anomaly_summary_data instanceof Error?ae.initialError=l.anomaly_summary_data:ae.initialData=l.anomaly_summary_data,l.anomaly_summary_columns&&(ae.knownColumns=l.anomaly_summary_columns));let K,ue=!1;const ke=se.createReactive({callback:V=>{a(3,K=V)},execFn:f},{id:"anomaly_summary",...ae});ke(B,{noResolve:b,...ae}),globalThis[Symbol.for("anomaly_summary")]={get value(){return K}};let pe={initialData:void 0,initialError:void 0},ne=me`select * from (with agency_yearly as (
    select
        agency_name,
        fiscal_year,
        sum(total_budget_amount) as spend
...
where abs(deviation_pct) > ${i.f_threshold.value??25}
  and cast(fiscal_year as varchar) like '${i.f_fy.value??"%"}'
order by abs(deviation_pct) desc) where anomaly_type = 'Spike' order by deviation_pct desc`;l.positive_anomalies_data&&(l.positive_anomalies_data instanceof Error?pe.initialError=l.positive_anomalies_data:pe.initialData=l.positive_anomalies_data,l.positive_anomalies_columns&&(pe.knownColumns=l.positive_anomalies_columns));let Te,ve=!1;const de=se.createReactive({callback:V=>{a(4,Te=V)},execFn:f},{id:"positive_anomalies",...pe});de(Ee,{noResolve:ne,...pe}),globalThis[Symbol.for("positive_anomalies")]={get value(){return Te}};let ye={initialData:void 0,initialError:void 0},re=me`select * from (with agency_yearly as (
    select
        agency_name,
        fiscal_year,
        sum(total_budget_amount) as spend
...
where abs(deviation_pct) > ${i.f_threshold.value??25}
  and cast(fiscal_year as varchar) like '${i.f_fy.value??"%"}'
order by abs(deviation_pct) desc
limit 50`;l.deep_anomalies_data&&(l.deep_anomalies_data instanceof Error?P.initialError=l.deep_anomalies_data:P.initialData=l.deep_anomalies_data,l.deep_anomalies_columns&&(P.knownColumns=l.deep_anomalies_columns));let Q,U=!1;const Me=se.createReactive({callback:V=>{a(6,Q=V)},execFn:f},{id:"deep_anomalies",...P});return Me(j,{noResolve:L,...P}),globalThis[Symbol.for("deep_anomalies")]={get value(){return Q}},s.$$set=V=>{"data"in V&&a(7,n=V.data)},s.$$.update=()=>{s.$$.dirty[0]&128&&a(8,{data:l={},customFormattingSettings:o,__db:c}=n,l),s.$$.dirty[0]&256&&zt.set(Object.keys(l).length>0),s.$$.dirty[1]&128&&e.params,s.$$.dirty[0]&15360&&(E||!G?E||(z(q,{noResolve:E,...y}),a(13,G=!0)):z(q,{noResolve:E})),s.$$.dirty[0]&245760&&(I||!F?I||(O(D,{noResolve:I,...R}),a(17,F=!0)):O(D,{noResolve:I})),s.$$.dirty[0]&512&&a(19,X=me`with agency_yearly as (
    select
        agency_name,
        fiscal_year,
        sum(total_budget_amount) as spend
...
where abs(deviation_pct) > ${i.f_threshold.value??25}
  and cast(fiscal_year as varchar) like '${i.f_fy.value??"%"}'
order by abs(deviation_pct) desc
limit 50`),s.$$.dirty[1]&120&&(L||!U?L||(Me(j,{noResolve:L,...P}),a(37,U=!0)):Me(j,{noResolve:L}))},a(11,E=me`select distinct agency_name from mbtsa.subprogram_level where agency_name is not null order by agency_name`),a(12,q="select distinct agency_name from mbtsa.subprogram_level where agency_name is not null order by agency_name"),a(15,I=me`select distinct fiscal_year as fy from mbtsa.subprogram_level order by fiscal_year`),a(16,D="select distinct fiscal_year as fy from mbtsa.subprogram_level order by fiscal_year"),[Y,M,ge,K,Te,Oe,Q,n,l,i,y,E,q,G,R,I,D,F,A,X,ce,le,ae,b,B,ue,pe,ne,Ee,ve,ye,re,qe,Se,P,L,j,U,e]}class $n extends Pe{constructor(t){super(),Le(this,t,on,sn,We,{data:7},null,[-1,-1])}}export{$n as component};
